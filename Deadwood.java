...
/*
 * 
 * Team: Carroll, Cruz, Ng, Yung CSCI 345: Deadwood Game Assignment Summer 2017
 *
 * File: Deadwood.java Libraries used: org.w3c.dom (.Document, .Element, .Node.,
 * .NodeList) java.util (.ArrayList, .Iterator, .Scanner) javax.xml.parsers
 ...
 * 
 * 
 * import java.util.*; import javax.xml.parsers.*; import org.w3c.dom.*; import
 * javax.swing.*; import javax.swing.JLayeredPane; import java.awt.Dimension;
 ...
 * Scanner in = new Scanner(System.in);
 * 
 * int n_player = View.numPlayer(); Board board = new Board();
 * board.setVisible(true);
 * 
 * 
 * Initializes the starting rank and credit for each player Starts the game for
 ...
 * 
 * 
 * int startRank = 1; int startCredit = 0; Player[] players = new
 * Player[n_player];
 * 
 * for (int i = 0; i < n_player; i++) { players[i] = new Player(i, startRank,
 * startCredit); }
 * 
 * playGame(players, getHowManyDays(n_player));
 * 
 * // Closes the scanner when the game is over in.close();
 * 
 * }
 * 
 * 
 ...
 * 
 * 
 * private static void playGame(Player[] players, int maxDays) throws Throwable
 * {
 * 
 * ArrayList<Card> cards = init_cards(); ArrayList<Room> rooms = init_rooms();
 * Room startPlace = findRoom(rooms, "Trailer"); int numPlayers =
 * players.length;
 * 
 * boolean roleTaken; boolean moved; boolean worked; int currPlayer; int
 * counter;
 * 
 * String command; String dest; String[] up;
 * 
 * // Keeps the game running through the final day
 * 
 * for (int day = 1; day <= maxDays; day++) { View.announce("DAY " + day);
 * 
 * // Sets the starting place on each day for (int k = 0; k < numPlayers; k++) {
 * players[k].setRoom(startPlace); }
 * 
 * // Sets the cards for each of the rooms Iterator<Room> iter =
 * rooms.iterator(); while (iter.hasNext()) {
 * 
 * Object temp = iter.next(); if
 * (temp.getClass().getSimpleName().toString().equals("Stage")) { ((Stage)
 * temp).setScene(cards); }
 * 
 * }
 * 
 * numSceneCardsOnBoard = 10; counter = -1; Scanner s = new Scanner(System.in);
 * 
 * 
 * I THINK that if we were to loop through all the rooms and counted the number
 * of scenes with 0 shots, then we wouldn't have to use the static variable
 ...
 * while (numSceneCardsOnBoard != 1) {
 * 
 * counter++; currPlayer = counter % numPlayers; worked = false; moved = false;
 * 
 * 
 * //System.out.print("Give me a command: ");
 * 
 * // ask for action command = null; dest = null; up = null;
 * 
 * // Parses user's input and acts according to command given do { command =
 * b.getCommand();
 * 
 * // Identifies current player and any parts the player is working if
 * (command.equals("who")) { players[currPlayer].who();
 * 
 * }
 * 
 * // Describes the current player's room and any active scenes else if
 * (command.equals("where")) { players[currPlayer].where(); }
 * 
 * 
 * Moves the current player to the indicated room Checks if it is a legal move
 * command: move {room}
 * 
 * 
 * else if (command.equals("move")) {
 * 
 * // if (moved == false){ // if (command.length > 1) { // dest =
 * command[1].toString(); // if (players[currPlayer].move(findRoom(rooms,
 * dest))){ // moved = true; // } // } // else { //
 * System.out.println("Please enter a valid work command"); // } // } // //
 * else{ // System.out.println("You have already moved this turn."); // }
 * 
 ...
 * dest = View.moveRoom(players[currPlayer].getRoom());
 * 
 * if (players[currPlayer].move(findRoom (rooms, dest))) { moved = true;
 * View.announce("Player has successfully moved to " + dest);
 * 
 * } }
 * 
 * else { View.announce("You have already moved this turn"); } break; }
 * 
 * 
 * Upgrades the player's current rank Checks if it is a legal upgrade command:
 * upgrade {$/credits} {level}
 ...
 * // this needs fixing!!!!!!!!!!!!!!!
 * 
 * String payType = View.upgradePayType(); int desiredRank = View.upgradeRank();
 * players[currPlayer].upgrade(payType, desiredRank);
 * 
 * 
 * // // if (command.length > 1) { // up = command[1].split(" ", 2); //
 * players[currPlayer].upgrade(up[0], Integer.parseInt(up[1])); // } // // else
 * { // System.out.println("Please enter a valid upgrade command"); // }
 * System.out.println("upgrade");
 * 
 * }
 ...
 * 
 * 
 * else if (command.equals("work")) { // roleTaken = false; // if (worked ==
 * false){ // if (command.length > 1) { // for (int i = 0; i < numPlayers; i++){
 * // if (players[i].getHasRole()){ // if //
 * (players[i].getRole().getName().toString().toLowerCase().equals(command[1].
 * toLowerCase())){ //
 * System.out.println("Another player already has this role."); // roleTaken =
 * true; // } // } // } // if (!roleTaken){ // if
 * (players[currPlayer].work(command[1])){ // worked = !worked; // } // } // }
 * else { // System.out.println("Please enter a valid work command"); // // if
 * (players[i].getRoom()) // } // } else{ //
 * System.out.println("You have already worked this turn."); // }
 * System.out.println("work");
 * 
 * }
 * 
 * // The current player rehearses else if (command.equals("rehearse")) {
 * 
 * if (worked == false){ if(players[currPlayer].rehearsal()){ worked = !worked;
 * } }
 * 
 * else { View.announce("You have already worked this turn."); }
 * 
 * System.out.println("rehearse"); }
 * 
 * // The current player performs their current role else if
 * (command.equals("act")) { if (worked == false) { boolean hadRole =
 * players[currPlayer].getHasRole(); if (players[currPlayer].act()) { worked =
 * true; } boolean hasRole = players[currPlayer].getHasRole();
 * 
 * if (hadRole && !hasRole) { if (playerOnCard(players, currPlayer))
 * players[currPlayer].wrap(players); else players[currPlayer].reset(); }
 * 
 * } else { View.announce("You have already worked this turn."); } }
 * 
 * // Current player wants to end their turn else if (command.equals("end")) {
 * 
 * // Player is only visiting room, not working on a role if
 * (players[currPlayer].getHasRole() == false) {
 * View.announce("Player was only visiting the room"); worked = !worked; moved =
 * false; }
 * 
 * //System.out.println("Changing to the next player\n");
 * View.announce("Changing to the next player"); }
 ...
 * }
 * 
 * // END GAME int max = players[0].getScore(); int maxi = 0; for (int a = 0; a
 * < players.length; a++) {
 * 
 * int score = players[a].getScore(); System.out.println(players[a].getName() +
 * " recieved " + score + " points.");
 * 
 * if (score > max) { max = score; maxi = a; }
 * 
 * }
 * 
 * System.out.println("The winner is " + players[maxi].getName() + " with " +
 * max + " points."); }
 * 
 * 
 * Finds a room given its name
 ...
 * 
 * private static Room findRoom(ArrayList<Room> rooms, String str) {
 * Iterator<Room> iter = rooms.iterator(); Room room = null; str =
 * str.toLowerCase();
 * 
 * while (iter.hasNext()) { room = iter.next(); if
 * (room.getroomName().toLowerCase().equals(str)) break; }
 * 
 * return (Room) room; }
 * 
 * 
 ...
 * builder.parse("board.xml"); NodeList children =
 * doc.getDocumentElement().getChildNodes(); ArrayList<Room> temp = new
 * ArrayList<Room>();
 * 
 ...
 * DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
 * DocumentBuilder builder = factory.newDocumentBuilder(); Document doc =
 * builder.parse("cards.xml"); NodeList children =
 * doc.getElementsByTagName("card"); ArrayList<Card> temp = new
 * ArrayList<Card>();
 * 
 * for (int i = 0; i < children.getLength(); ++i) { Element card = (Element)
 * children.item(i); temp.add(new Card(card)); }
 * 
 * return temp; }
 * 
 * 
 ...
 * Player p = player[ind];
 * 
 * for (int i = 0; i < player.length; i++) { if (p.getonRole() &&
 * p.getRoom().getroomName().equals(player[i].getRoom().getroomName())) { return
 * true; } }
 * 
 * return false; }
 * 
 ...
 * 
 * 
 * private static boolean isNum(String s) { return
 * s.matches("[-+]?\\d*\\.?\\d+"); }
 * 
 * // Returns if a number is in the range between a minimum and maximum number
 * private static boolean isValidRange(int num, int min, int max) { return min
 * <= num && num <= max; } }
 */